...
for trial = 1:length(trials)
    ...
    for cellType = 1:2;
        ...
        for onset = 1:length(onsets)          
            ...
            ampOnsetMatCell = zeros(size(onsetMat,1),length(window)-1);
            for nCell = 1:size(onsetMat,1)
                ...
                for bin = 1:length(window)-1
                    ampOnsetMatCell(nCell,bin) = mean(tempPKS(tempLOCS >= window(bin) & tempLOCS < window(bin+1)));
                end               
            end
        end      
        spikeHist{cellType}(trial,:) = nanmean(ampOnsetMatCell,1);
...
figure;
hold on
smoothing = 1; %smoothing = 5 for continuous graph
for cellType = 1:2
%     window = 0:.005:.25;
    if cellType == 1
...
%     end
    title('Amplitude of Spikes in SPNs At Onset');
    xlabel('Time (s)');
