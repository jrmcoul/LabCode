...
mouse = {'F028','F030','F031','F032','F034','F035','F036','F037',};
date = {'180129','180130',};
acq = {'1','2'};
...
newDirectory = cat(2,'L:\tritsn01labspace\Marta\Fiberphotometry\',data.imageFile,'\',data.date,'\');
cd(newDirectory)
...
[h, edges] = histcounts(abs(fourierFilt(data.FP,5,data.sampleRate,'low')),1000);
ind = find(h > 50,1);
data.FPNorm = (data.FP - edges(ind))./edges(ind);
data.FPNorm = abs(fourierFilt(data.FPNorm,5,data.sampleRate,'low'));
