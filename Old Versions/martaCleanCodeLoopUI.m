...
path = {'C:\MATLAB\Marta\SW034\171002 acq2\'};

for trial = 1:length(path)

cd(path{trial});

data = struct; % Defining data as an empty structure

data.imageFile = path{trial}(17:21); %CHANGE
data.date = path{trial}(23:28); %CHANGE
...
clearvars -except data trial path
close all
...
firstFile = str2num(path{trial}(33)); %CHANGE
