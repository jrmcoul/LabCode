%% What we have:
% freqRest10, freqMov10 - frequency during rest and movement for each cell
...
% motorSpontDiffRest, motorSpontDiffMov - difference between motor and spontaneous spike frequencies
...
for cellType = 1:2
    for i = 1:3
        transitions{1,cellType}(i) = cellsAddedSame{1,cellType}(i)/cellsAddedSame{4,cellType}(i);
        transitions{2,cellType}(i) = cellsAddedSame{3,cellType}(i)/cellsAddedSame{4,cellType}(i);
    end
end
...
for cellType = 1:2
    for i = 1:3
        transitions{1,cellType}(i) = cellsAddedSame{1,cellType}(i)/cellsAddedSame{4,cellType}(i);
        transitions{2,cellType}(i) = cellsAddedSame{2,cellType}(i)/cellsAddedSame{4,cellType}(i);
    end
end
%%
% 
